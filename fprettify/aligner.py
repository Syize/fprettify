...
from .exception import FprettifyInternalException
...
class F90Aligner(object):
    ...
    def process_lines_of_fline(self, f_line, lines, rel_ind, line_nr):
        ...
        is_decl = (
            re_str.VAR_DECL_RE.search(f_line)
            or re_str.PUBLIC_RE.search(f_line)
            or re_str.PRIVATE_RE.match(f_line)
        )
        is_use = re_str.USE_RE.search(f_line)
    ...
    def __align_line_continuations(self, line, is_decl, is_use, indent_size, line_nr):
        ...
        for pos, char in CharFilter(line):
            ...
            if (
                ...
                and not is_decl
                and char == "="
                and not re_str.REL_OP_RE.search(
                ...
                # line break
                if not re.search(r"=>?\s*" + re_str.LINEBREAK_STR, line, re_str.RE_FLAGS):
                    indent_list.append(pos_eq + 1 + is_pointer + indent_list[-1])
            elif (
                is_decl
                and line[pos : pos + 2] == "::"
                and not re.search(r"::\s*" + re_str.LINEBREAK_STR, line, re_str.RE_FLAGS)
            ):
        ...
        if level and re.search(re_str.DEL_OPEN_STR + r"\s*" + re_str.LINEBREAK_STR, line, re_str.RE_FLAGS):
